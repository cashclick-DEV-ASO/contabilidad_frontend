export const anchoBBVA = (texto, layout) => {
	...
	lineas.forEach((texto, linea) => {
		...
		if (idRegistro === idRegReg) {
			const idMovimiento = getIdentificador(
				texto,
				tandemReg.inicio,
				tandemReg.espacios
			)
			...
			if (unificado.length === 2) {
				resultado.movimientos.push(
					extraeCampos(unificado.join(""), camposReg)
				)
				unificado = []
				return
			}
		...
		if (idRegistro === idRegCierre) {
			const idMovimiento = getIdentificador(
				texto,
				tandemCierre.inicio,
				tandemCierre.espacios
			)
			unificado = evaluaTandem(
				idMovimiento,
				unificado,
				texto,
				tandemCierre
			...
			if (unificado.length === 2) {
				resultado.informacion.cierre = extraeCampos(
					unificado.join(""),
					camposCierre
				)
				unificado = []
