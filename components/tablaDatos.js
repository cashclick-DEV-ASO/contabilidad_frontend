...
export class TablaDatos extends Componente {
	constructor(filtro = true) {
		super(SYS.SCTN, { id: TABLA.CONTENEDOR })
		this.encabezados = []
		this.filas = []
		this.filtro = filtro
		this.detalles = {
	...
	inicia() {
		...
		this.contruirTabla(encabezados, filas)
		return this
	}
	...
	contruirTabla(encabezados = this.encabezados, filas = this.filas) {
		this.tabla = new Componente(SYS.TBL, { id: TABLA.ID_TABLA })
		...
		tblEncabezdos.addHijo(
			new Componente(SYS.TR, { clase: TABLA.TBL_FILA_HDR })
				.addHijos(
					encabezados.map(e =>
						new Componente(SYS.TH, { clase: TABLA.TBL_CELDA_HDR })
		...
		tblCuerpo.addHijos(
			filas.map(f => {
				...
				fila.addHijos(
					f.map(c =>
						new Componente(SYS.TD, { clase: TABLA.TBL_CELDA })
							.setTexto(c)
							.getComponente()
					)
				)
	...
	mostrarDetalles() {
		...
		Object.keys(this.detalles).forEach(key => {
			const contenedor = new Componente(SYS.DIV, {
				clase: TABLA.DETALLE,
			})

			const lblTitulo = new Componente(SYS.LBL, {
				clase: TABLA.LBL_DETALLE,
			})
			const lblValor = new Componente(SYS.LBL, {
				clase: TABLA.TXT_DETALLE,
			})
