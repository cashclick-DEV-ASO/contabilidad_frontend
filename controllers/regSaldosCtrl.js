...
export class RegSaldosCtrl extends Controlador {
	...
	limpiaCampos = ({ cta = true, bnk = false } = {}) => {
	...
	verificarDatos = async (cerrar = null) => {
		...
		this.modelo.registraSaldo(sldo).then(resultado => {
			if (resultado.success) {
				...
				this.msjError(resultado.informacion.mensaje)
			}
			if (cerrar) cerrar()
		})
	}
	...
	formatoMoneda = (input, blur) => {
		...
		if (input_val.indexOf(".") >= 0) {
			...
			if (blur === SYS.BLR) right_side += "00"
			right_side = right_side.substring(0, 2)
			input_val = "$" + left_side + "." + right_side
		} else {
			input_val = this.formatoNumero(input_val)
			input_val = "$" + input_val
			if (blur === SYS.BLR) input_val += ".00"
		}
